...
void    ClapTrap::beRepaired(unsigned int amount) {
    if (this->_Energy_points == 0 || this->_Hit_points == 0) {
        ...
        std::cout << "ClapTrap " << this->_Name << " can't repair himself because he is already at max Hit points!" << std::endl;
    }
    else {
        this->_Energy_points--;
        if (this->_Hit_points + amount <= 10) {
            ...
            std::cout << "ClapTrap " << this->_Name << " repairs for a total of " << 10 - this->_Hit_points << " health!" << std::endl;
            this->_Hit_points = 10;
        }
    }
}